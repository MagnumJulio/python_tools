...
def compute_diffusion(df: pd.DataFrame, leaves: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cobertura minima pra evitar valores enganosos em meses com release BLS
    # parcial (ex.: Out-2025 tinha so 6/177 items por shutdown do governo US).
    # Se um mes tem < 70% dos items com YoY valido, retorna NaN nesse mes.
    def diffusion(rows: pd.DataFrame, col: str) -> pd.Series:
        # Contagem esperada = # unico de items no scope (fixed no periodo).
        expected = rows.item_code.nunique()
