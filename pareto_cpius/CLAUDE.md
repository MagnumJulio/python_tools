...
6. **Loader SQL corp Itaú** (`script_itau/load_cpius_to_sql.py`, ✅ entregue): mesmo padrão de `pareto_ipca/script_itau/load_pareto_to_sql.py`. Cada categoria vira até 6 séries: NSA var, NSA idx, SA var, SA idx, Weight (label), Weight (label Index). Convenções seguem `pareto_ipca` sincronizado 2026-07-17: `data_type="Weight"` (não `"Peso"`), peso gravado **2×** por categoria — uma com `series_name=label` (par com var NSA/SA) e outra com `series_name=f"{label} (Index)"` (par com idx NSA/SA); `bls_code` do 2º leva sufixo `/Index`. Headline `all_items` recebe peso sintético 100.0 constante (sobrescreve valor renormalizado ~99.9997 do CSV). Proveniência migrou do campo `haver_code` (formato antigo `CPIUS:{cat}/{sid}/BLS-{sha}` + variantes `/CUSTOM/`, `/RI/`, `/Peso/`, `/Index`) pro campo **`bls_code`** (novo formato simplificado `CPIUS:{cat}` ou `CPIUS:{cat}/Index`); INSERTs novos não setam `haver_code` (o SQL preenche como `NULL` por default). Base e custom compartilham o mesmo formato — a distinção sai do próprio `category_code` (ex.: `core_ex_oer` vs `core`), não do code. Migração idempotente rodada antes do main loop: linhas antigas (`haver_code LIKE 'CPIUS:%'`) sofrem `UPDATE` explícito `SET haver_code = NULL, bls_code = <novo>`. Total: 47 × 6 = **282 séries**. Simulador local em `simulate_cpius_to_sql.py` (mesma lógica com `MockSQLConnector`).
