...
Fork do `pareto_ipca` para o **IPCA-15** (preview de meio de mês do IPCA, publicado ~dia 24 do mês de referência — adianta o IPCA cheio). Reconstrói o **headline (`total`) + 39 séries** derivadas (administrados, livres, industriais, serviços, alimentação no domicílio, núcleos, decomposições por processamento e por comercialização + 7 grupos IPCA G1/G2/G3/G4/G5/G7/G8 + alim_fora SG12, energia_eletrica I2202, passagem_aerea/auto_novo/auto_usado subitens), **100% a partir de IBGE/SIDRA**. Grupos e subgrupos/itens/subitens específicos são extraídos direto do SIDRA (não recon Laspeyres) — IBGE publica esses agregados prontos via `classificacao=315[all]`. Objetivo: entregar os números no mesmo dia do release IBGE.
