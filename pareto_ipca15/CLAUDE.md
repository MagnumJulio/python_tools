...
Fork do `pareto_ipca` para o **IPCA-15** (preview de meio de mês do IPCA, publicado ~dia 24 do mês de referência — adianta o IPCA cheio). Reconstrói o **headline (`total`) + 27 séries** derivadas (administrados, livres, industriais, serviços, alimentação no domicílio, núcleos, decomposições por processamento e por comercialização), **100% a partir de IBGE/SIDRA**. Objetivo: entregar os números no mesmo dia do release IBGE.
