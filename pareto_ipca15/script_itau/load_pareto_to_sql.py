...
# Carrega as 44 categorias do pareto_ipca15 (indice + peso) na base SQL corp
...
# data/ipca15_pareto_indice.csv (e opcionalmente data/ipca15_pareto_pesos.csv).
#
# Sync 2026-08-27: variacao mensal (NSA+SA) foi removida do SQL — mesma decisao
# do CPI-US 2026-07-20 e do pareto_ipca (mesma data). Cada categoria agora vira
# ate 3 series: idx NSA, idx SA (com --sa) e Weight (1x, pareado com idx).
# Rows de var pre-existentes no SQL corp ficam orphan — cleanup separado por
# escopo.
#
...
# rows do pareto_ipca no mesmo SQL. Todas as 1-3 rows por cat (idx NSA,
# opc. idx SA, opc. Weight) compartilham o mesmo bls_code.
#
...
# Sync 2026-08-27: var (NSA+SA) removida. Rows de var pre-existentes ficam
...
def main():
    ...
    try:
        ...
        max_desc = max(
            len(f"{CATEGORY_LABELS[c]} - Indice (dez/2012=100) - IPCA-15 recon IBGE-only via NT_57/Dez-2025")
            for c in cats
        )
