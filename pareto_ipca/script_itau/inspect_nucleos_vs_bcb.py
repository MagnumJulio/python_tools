...
# Overlay focado dos 3 nucleos estatisticos contra BCB SGS, pra diagnosticar
# onde nossa recon diverge das series oficiais.
#
# SGS: nucleo_ma=11426, nucleo_ms=4466, nucleo_dp=16122
#
# Outputs em scripts/outputs/inspection/:
#   nucleos_overlay.png    - 3 paineis: nossa vs BCB (variacao mensal)
#   nucleos_diff.png       - 3 paineis: ours - bcb (pp), com banda +-0.1pp
#   nucleos_overlay_12m.png- 3 paineis: ours vs BCB acumulada 12m
#
# Uso:
#   cd pareto_ipca
...
def main():
    ...
    for ax, (cat, _, title) in zip(axes, NUCLEOS):
        ...
        ax.set_ylabel("var. mensal (%)")
    fig.suptitle("Nucleos estatisticos: nossa recon vs BCB SGS (variacao mensal)", fontsize=13, y=1.0)
    fig.tight_layout()
    ...
    for ax, (cat, _, title) in zip(axes, NUCLEOS):
        ...
        ax.set_title(f"{title}  (ours - BCB)", fontsize=11)
        ax.set_ylabel("diff (pp)")
        ax.grid(alpha=0.3)
        ax.legend(loc="upper right", fontsize=9)
